...
func addTestnetFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(sdkserver.FlagMinGasPrices,
		fmt.Sprintf("0.000006%s",
			mrmintchain.AttoPhoton),
		"Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01photino,0.001mnt)")
...
Example:
	...
	cmd.Flags().String(flagNodeDaemonHome, "mrmintchaind", "Home directory of the node's daemon configuration")
...
func initTestnetFiles(
	...
	if args.chainID == "" {
		args.chainID = fmt.Sprintf("mrmintchain_%d-1", tmrand.Int63n(9999999999999)+1)
	}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		accStakingTokens := sdk.TokensFromConsensusPower(5000, mrmintchain.PowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin(mrmintchain.AttoPhoton, accStakingTokens),
		}

		genBalances = append(genBalances, banktypes.Balance{Address: addr.String(), Coins: coins.Sort()})
		genAccounts = append(genAccounts, &mrmintchain.EthAccount{
			BaseAccount: authtypes.NewBaseAccount(addr, nil, 0, 0),
			CodeHash:    common.BytesToHash(evmtypes.EmptyCodeHash).Hex(),
		})

		valTokens := sdk.TokensFromConsensusPower(100, mrmintchain.PowerReduction)
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(mrmintchain.AttoPhoton, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(mrmintchain.AttoPhoton)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, mrmintchain.AttoPhoton, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
