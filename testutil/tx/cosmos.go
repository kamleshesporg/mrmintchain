...
type CosmosTxArgs struct {
	...
	// ChainID is the chain's id on cosmos format, e.g. 'mrmintchain_9000-1'
	ChainID string
...
func PrepareCosmosTx(
	ctx sdk.Context,
	appmrmintchain *app.mrmintchainApp,
	args CosmosTxArgs,
...
func signCosmosTx(
	...
	seq, err := appmrmintchain.AccountKeeper.GetSequence(ctx, addr)
	if err != nil {
		return nil, err
	}
	...
	accNumber := appmrmintchain.AccountKeeper.GetAccount(ctx, addr).GetAccountNumber()
