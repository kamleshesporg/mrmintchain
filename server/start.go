...
import (
	...
	"github.com/kamleshesporg/mrmintchain/indexer"
	ethdebug "github.com/kamleshesporg/mrmintchain/rpc/namespaces/ethereum/debug"
...
`,
	...
	cmd.Flags().String(server.FlagMinGasPrices, "", "Minimum gas prices to accept for transactions; Any fee in a tx must meet this minimum (e.g. 0.01photon;0.0001mnt)") //nolint:lll
	...
	cmd.Flags().Uint64(srvflags.JSONRPCGasCap, config.DefaultGasCap, "Sets a cap on gas that can be used in eth_call/estimateGas unit is mnt (0=infinite)")         //nolint:lll
...
func startInProcess(ctx *server.Context, clientCtx client.Context, opts StartOptions) (err error) {
	...
	var idxer mrmintchain.EVMTxIndexer
