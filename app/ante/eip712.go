...
var mrmintchainCodec codec.ProtoCodecMarshaler

func init() {
	registry := codectypes.NewInterfaceRegistry()
	mrmintchain.RegisterInterfaces(registry)
	mrmintchainCodec = codec.NewProtoCodec(registry)
}
...
func VerifySignature(
	...
	case *signing.SingleSignatureData:
		...
		signerChainID, err := mrmintchain.ParseChainID(signerData.ChainID)
		if err != nil {
		...
		extOpt, ok := opts[0].GetCachedValue().(*mrmintchain.ExtensionOptionsWeb3Tx)
		if !ok {
		...
		typedData, err := eip712.LegacyWrapTxToTypedData(mrmintchainCodec, extOpt.TypedDataChainID, msgs[0], txBytes, feeDelegation)
		if err != nil {
