...
func (egcd EthGasConsumeDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	if ctx.IsReCheckTx() {
		...
		// https://github.com/kamleshesporg/mrmintchain/issues/1554
		newCtx := ctx.WithGasMeter(mrmintchain.NewInfiniteGasMeterWithLimit(gasWanted))
	...
	blockGasLimit := mrmintchain.BlockGasLimit(ctx)
	...
	newCtx := ctx.
		WithGasMeter(mrmintchain.NewInfiniteGasMeterWithLimit(gasWanted)).
