...
func SetBip44CoinType(config *sdk.Config) {
	config.SetCoinType(mrmintchain.Bip44CoinType)
	config.SetPurpose(sdk.Purpose)                        // Shared
	config.SetFullFundraiserPath(mrmintchain.BIP44HDPath) //nolint: staticcheck
}
...
func RegisterDenoms() {
	...
	if err := sdk.RegisterDenom(mrmintchain.AttoPhoton, sdk.NewDecWithPrec(1, mrmintchain.BaseDenomUnit)); err != nil {
		panic(err)
	}
}
