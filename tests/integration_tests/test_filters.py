...
from .network import setup_custom_mrmintchain, setup_mrmintchain
from .utils import (
    ADDRS,
    CONTRACTS,
    deploy_contract,
...
def custom_mrmintchain(tmp_path_factory):
    ...
    yield from setup_mrmintchain(path, 26200, long_timeout_commit=True)


@pytest.fixture(scope="module")
def mrmintchain_indexer(tmp_path_factory):
...
@pytest.fixture(
    scope="module", params=["mrmintchain", "geth", "mrmintchain-ws", "enable-indexer"]
)
def cluster(request, custom_mrmintchain, mrmintchain_indexer, geth):
    """
