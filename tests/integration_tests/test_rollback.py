...
from .network import setup_custom_mrmintchain
...
def update_node_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:mrmintchain_9000-1-node{i}":
            ini[section].update(
                {
...
def custom_mrmintchain(tmp_path_factory):
    ...
    cmd = [
        "nix-build",
        "--no-out-link",
        Path(__file__).parent / "configs/broken-mrmintchaind.nix",
    ]
    print(*cmd)
    broken_binary = (
        Path(subprocess.check_output(cmd).strip().decode()) / "bin/mrmintchaind"
    )
    print(broken_binary)
...
def test_rollback(custom_mrmintchain):
    """
    ...
    print("stop node1")
    supervisorctl(
        custom_mrmintchain.base_dir / "../tasks.ini", "stop", "mrmintchain_9000-1-node1"
    )
    ...
    update_node_cmd(custom_mrmintchain.base_dir, "mrmintchaind", 1)
    supervisorctl(custom_mrmintchain.base_dir / "../tasks.ini", "update")
    wait_for_port(target_port)
