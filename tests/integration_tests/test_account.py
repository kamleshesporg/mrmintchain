...
def custom_mrmintchain(tmp_path_factory):
    ...
    yield from setup_mrmintchain(path, 26700, long_timeout_commit=True)


@pytest.fixture(scope="module", params=["mrmintchain", "mrmintchain-ws", "geth"])
def cluster(request, custom_mrmintchain, geth):
    """
    run on mrmintchain, mrmintchain websocket and geth
    """
