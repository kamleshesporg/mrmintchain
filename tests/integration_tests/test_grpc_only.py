...
def grpc_eth_call(port: int, args: dict, chain_id=None, proposer_address=None):
    ...
    return requests.get(
        f"http://localhost:{port}/mrmintchain/evm/v1/eth_call", params
    ).json()


def test_grpc_mode(custom_mrmintchain):
    """
    ...
    api_port = ports.api_port(custom_mrmintchain.base_port(1))
    ...
    with (custom_mrmintchain.base_dir / "node1.log").open("a") as logfile:
        proc = subprocess.Popen(
            [
                ...
                "--grpc-only",
                "--home",
                custom_mrmintchain.base_dir / "node1",
            ],
            stdout=logfile,
        ...
        try:
            ...
            grpc_port = ports.grpc_port(custom_mrmintchain.base_port(1))
            ...
            addr = custom_mrmintchain.cosmos_cli(0).consensus_address()
