...
DEFAULT_CHAIN_BINARY = "mrmintchaind"
...
class mrmintchain:
    ...
    def copy(self):
        return mrmintchain(self.base_dir)
...
def setup_mrmintchain(path, base_port, long_timeout_commit=False):
    cfg = Path(__file__).parent / (
        "configs/default.jsonnet"
        if long_timeout_commit
        else "configs/long_timeout_commit.jsonnet"
    )
    yield from setup_custom_mrmintchain(path, base_port, cfg)
...
def setup_custom_mrmintchain(
    ...
    try:
        ...
        yield mrmintchain(
            path / "mrmintchain_9000-1", chain_binary=chain_binary or DEFAULT_CHAIN_BINARY
        )
    finally:
        os.killpg(os.getpgid(proc.pid), signal.SIGTERM)
        proc.wait()
