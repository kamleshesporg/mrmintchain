...
from .network import mrmintchain, setup_custom_mrmintchain
from .utils import (
    ADDRS,
    CONTRACTS,
    deploy_contract,
            ...
            ini[section].update(
                {
                    ...
                    "environment": (
                        f"DAEMON_NAME=mrmintchaind,DAEMON_HOME=%(here)s/node{i}"
                    ),
                }
            )
...
def custom_mrmintchain(tmp_path_factory):
    ...
    yield from setup_custom_mrmintchain(
        path,
        26100,
        Path(__file__).parent / "configs/cosmovisor.jsonnet",
        post_init=post_init,
        chain_binary=str(path / "upgrades/genesis/bin/mrmintchaind"),
    )


def test_cosmovisor_upgrade(custom_mrmintchain: mrmintchain):
    """
    ...
    """
    cli = custom_mrmintchain.cosmos_cli()

    w3 = custom_mrmintchain.w3
    ...
    assert rsp["code"] == 0, rsp["raw_log"]
    # rsp = custom_mrmintchain.cosmos_cli(1).gov_vote("validator", proposal_id, "yes")
    # assert rsp["code"] == 0, rsp["raw_log"]
    ...
    custom_mrmintchain.chain_binary = (
        Path(custom_mrmintchain.chain_binary).parent.parent.parent
        / f"{plan_name}/bin/mrmintchaind"
    )
    cli = custom_mrmintchain.cosmos_cli()

    # block should pass the target height
    wait_for_block(cli, target_height + 1, timeout=480)
    wait_for_port(ports.rpc_port(custom_mrmintchain.base_port(0)))

    # test migrate keystore
    cli.migrate_keystore()

    # check basic tx works after upgrade
    wait_for_port(ports.evmrpc_port(custom_mrmintchain.base_port(0)))
