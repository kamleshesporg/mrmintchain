...
function checkTestEnv() {
  ...
  const argv = yargs(hideBin(process.argv))
    ...
    .example('$0 --network mrmintchain', 'run all tests using mrmintchain network')
    .example('$0 --network mrmintchain --allowTests=test1,test2', 'run only test1 and test2 using mrmintchain network')
    .help('h').alias('h', 'help')
    .describe('network', 'set which network to use: ganache|mrmintchain')
    ...
    .boolean('verbose-log').describe('verbose-log', 'print mrmintchaind output, default false')
    .argv;
  ...
  if (!argv.network) {
    ...
    if (argv.network !== 'mrmintchain' && argv.network !== 'ganache') {
      panic('network is invalid. Must be ganache or mrmintchain');
    }
    else {
...
function loadTests(runConfig) {
  ...
  fs.readdirSync(path.join(__dirname, 'suites')).forEach(dirname => {
    ...
    try {
      ...
      const needScripts = ['test-ganache', 'test-mrmintchain'];
...
function performTestSuite({ testName, network }) {
  const cmd = network === 'ganache' ? 'test-ganache' : 'test-mrmintchain';
...
function setupNetwork({ runConfig, timeout }) {
  if (runConfig.network !== 'mrmintchain') {
    // no need to start ganache. Truffle will start it
    return;
  }

  // Spawn the mrmintchain process

  const spawnPromise = new Promise((resolve, reject) => {
    const mrmintchaindProc = spawn('./init-test-node.sh', {
      cwd: __dirname,
      stdio: ['ignore', runConfig.verboseLog ? 'pipe' : 'ignore', 'pipe'],
    });

    logger.info(`Starting mrmintchaind process... timeout: ${timeout}ms`);
    if (runConfig.verboseLog) {
      mrmintchaindProc.stdout.pipe(process.stdout);
    }
    mrmintchaindProc.stderr.on('data', d => {
      const oLine = d.toString();
      if (runConfig.verboseLog) {
        process.stdout.write(oLine);
      }
      if (oLine.indexOf('Starting JSON-RPC server') !== -1) {
        logger.info('mrmintchaind started');
        resolve(mrmintchaindProc);
      }
    });
  });

  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start mrmintchaind timeout!')), timeout);
  });
