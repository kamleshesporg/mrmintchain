...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
mrmintchain_BINARY = mrmintchaind
mrmintchain_DIR = mrmintchain
BUILDDIR ?= $(CURDIR)/build
SIMAPP = ./app
HTTPS_GIT := https://github.com/kamleshesporg/mrmintchain.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=mrmintchain \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=$(mrmintchain_BINARY) \
...
docker-build:
	...
	docker rm mrmintchain || true
	# create a new container from the latest image
	docker create --name mrmintchain -t -i tharsis/mrmintchain:latest mrmintchain
	# move the binaries to the ./build directory
	mkdir -p ./build/
	docker cp mrmintchain:/usr/bin/mrmintchaind ./build/
...
PACKAGE_NAME:=github.com/kamleshesporg/mrmintchain
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/kamleshesporg/mrmintchain/types"
	godoc -http=:6060
...
test-import:
	go test -run TestImporterTestSuite -v --vet=off github.com/kamleshesporg/mrmintchain/tests/importer
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(mrmintchain_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\mrmintchain\Z mrmintchaind/node "./mrmintchaind testnet --v 4 -o /mrmintchain --keyring-backend=test --ip-addresses mrmintchaindnode0,mrmintchaindnode1,mrmintchaindnode2,mrmintchaindnode3"
	docker-compose up -d
else
	mkdir -p localnet-setup
	@$(MAKE) localnet-build

	if ! [ -f localnet-setup/node0/$(mrmintchain_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/localnet-setup:/mrmintchain:Z mrmintchaind/node "./mrmintchaind testnet --v 4 -o /mrmintchain --keyring-backend=test --ip-addresses mrmintchaindnode0,mrmintchaindnode1,mrmintchaindnode2,mrmintchaindnode3"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\localnet-setup\node0\ethermitd:mrmintchain\Z mrmintchaind/node "./mrmintchaind unsafe-reset-all --home=/mrmintchain"
	@docker run --rm -v $(CURDIR)\localnet-setup\node1\ethermitd:mrmintchain\Z mrmintchaind/node "./mrmintchaind unsafe-reset-all --home=/mrmintchain"
	@docker run --rm -v $(CURDIR)\localnet-setup\node2\ethermitd:mrmintchain\Z mrmintchaind/node "./mrmintchaind unsafe-reset-all --home=/mrmintchain"
	@docker run --rm -v $(CURDIR)\localnet-setup\node3\ethermitd:mrmintchain\Z mrmintchaind/node "./mrmintchaind unsafe-reset-all --home=/mrmintchain"
else
	@docker run --rm -v $(CURDIR)/localnet-setup/node0/ethermitd:/mrmintchain:Z mrmintchaind/node "./mrmintchaind unsafe-reset-all --home=/mrmintchain"
	@docker run --rm -v $(CURDIR)/localnet-setup/node1/ethermitd:/mrmintchain:Z mrmintchaind/node "./mrmintchaind unsafe-reset-all --home=/mrmintchain"
	@docker run --rm -v $(CURDIR)/localnet-setup/node2/ethermitd:/mrmintchain:Z mrmintchaind/node "./mrmintchaind unsafe-reset-all --home=/mrmintchain"
	@docker run --rm -v $(CURDIR)/localnet-setup/node3/ethermitd:/mrmintchain:Z mrmintchaind/node "./mrmintchaind unsafe-reset-all --home=/mrmintchain"
endif
...
.PHONY: build-docker-local-mrmintchain localnet-start localnet-stop
