...
DATA_DIR=$(mktemp -d -t mrmintchain-datadir.XXXXX)
...
init_func() {
    "$PWD"/build/mrmintchaind keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test mrmintchain node rpc=$HOST_RPC ws=$HOST_WS ..."
